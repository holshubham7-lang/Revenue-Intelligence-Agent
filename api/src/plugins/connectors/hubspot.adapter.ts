...
export class HubSpotAdapter implements ConnectorAdapter {
  ...
  buildAuthorizeUrl({ state, codeChallenge, redirectUri }: AuthorizeUrlParams): string {
  ...
  private async requestToken(
    ...
    if (!response.ok) {
      this.logger.error(
        `HubSpot ${flow} token request failed (${response.status}): ${text.slice(0, 300)}`,
      );
      throw new Error(`HubSpot token request failed (HTTP ${response.status})`);
    }
